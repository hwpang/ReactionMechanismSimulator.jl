...
@testset "Model reduction and simulation of reduced model" begin
	...
	bsol_reduced, unlumpedsol = Simulation(sol,domain,reducedmodelmappings,[],p);
	...
	@test unlumpedsol(1e-6)[ethaneind] ≈ bsol.sol(1e-6)[ethaneind]
	
	isomer1_ind = findfirst(isequal("[CH2]CC=C"),spcnames)
	isomer2_ind = findfirst(isequal("[CH]=CCC"),spcnames)
	
	@test unlumpedsol(1e-6)[isomer1_ind] ≈ unlumpedsol(1e-6)[isomer2_ind]
end