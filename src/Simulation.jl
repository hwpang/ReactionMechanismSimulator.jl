...
function Simulation(sol::Q,domain::W,reducedmodelmappings::ReducedModelMappings,interfaces=[],p=nothing) where {Q<:AbstractODESolution,W<:AbstractDomain} 
    ...
    unlumpedsol(t::T) where {T<:Real} = unlumpsol(t,sol,domain,reducedmodelmappings,reducedmodelcache,interfaces,p)

    species_range = 1:(length(reducedmodelmappings.reducedindexes)+length(reducedmodelmappings.lumpedgroupmapping))
    names = getfield.(domain.phase.species,:name)
    ...
    return Simulation(sol,domain,interfaces,names,F,Ns,domain.phase.species,domain.phase.reactions,p), unlumpedsol
end
