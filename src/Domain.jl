...
@inline function calcthermo(d::ConstantTrhoDomain{W,Y},y::J,t::Q,p::Q2=DiffEqBase.NullParameters()) where {Q2<:DiffEqBase.NullParameters,W<:IdealDiluteSolution,Y<:Integer,J<:AbstractArray,Q}
    ns = y[d.indexes[1]:d.indexes[2]]
    N = sum(ns)
    mass = y[d.indexes[3]]
    V = mass/d.rho
...
@inline function calcthermo(d::ConstantTrhoDomain{W,Y},y::J,t::Q,p::Q2=DiffEqBase.NullParameters()) where {Q2<:Array{Float64,1},W<:IdealDiluteSolution,Y<:Integer,J<:Array{Float64,1},Q}
    ns = y[d.indexes[1]:d.indexes[2]]
    N = sum(ns)
    mass = y[d.indexes[3]]
    V = mass/d.rho
...
@inline function calcthermo(d::ConstantTrhoDomain{W,Y},y::Array{W2,1},t::Q,p::Q2=DiffEqBase.NullParameters()) where {W2<:ForwardDiff.Dual,Q2,W<:IdealDiluteSolution,Y<:Integer,J<:AbstractArray,Q} #autodiff y
    ns = y[d.indexes[1]:d.indexes[2]]
    N = sum(ns)
    mass = y[d.indexes[3]]
    V = mass/d.rho
...
@inline function calcthermo(d::ConstantTrhoDomain{W,Y},y::J,t::Q,p::Q2=DiffEqBase.NullParameters()) where {Q2,W<:IdealDiluteSolution,Y<:Integer,J<:AbstractArray,Q} #autodiff p
    ns = y[d.indexes[1]:d.indexes[2]]
    N = sum(ns)
    mass = y[d.indexes[3]]
    V = mass/d.rho
...
@inline function calcdomainderivatives!(d::ConstantVDomain{W,Y},dydt::K,interfaces::Z12;t::Z10,T::Z4,P::Z9,Us::Z,Hs::Z11,V::Z2,C::Z3,ns::Z5,N::Z6,Cvave::Z7) where {Z12,Z11,Z10,Z9,W<:IdealGas,Z7,K,Y<:Integer,Z6,Z,Z2,Z3,Z4,Z5}
    ...
    for inter in interfaces
        ...
        elseif isa(inter,TPDependentOutlet) && d == inter.domain
            flow = inter.F/inter.P*P*inter.T/T
            dydt[d.indexes[1]:d.indexes[2]] .-= flow.*ns./N
            dTdt = (P*V/N*flow)/(N*Cvave)
            dydt[d.indexes[3]] -= dTdt
            dydt[d.indexes[4]] -= flow*R*T/V + P/T*dTdt
...
@inline function jacobiany!(jac::Q,y::U,p::W,t::Z,domain::D,interfaces::Q3,colorvec::Q2=nothing) where {Q3<:AbstractArray,Q2,Q<:AbstractArray,U<:AbstractArray,W,Z<:Real,D<:ConstantVDomain}
    ...
    for inter in interfaces
        ...
        elseif isa(inter,TPDependentOutlet) && domain == inter.domain
            flow = inter.F/inter.P*P*inter.T/T
            # dydt[d.indexes[1]:d.indexes[2]] .-= flow.*ns./N
            # dTdt = (P*V/N*flow)/(N*Cvave)
            # dydt[d.indexes[3]] -= dTdt
